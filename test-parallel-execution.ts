...
Always provide a summary of what operations were performed and their execution strategy.`;
  ...
  // Save agent
  await fs.mkdir(path.join(__dirname, 'agents'), { recursive: true });
  await fs.writeFile(
    path.join(__dirname, 'agents', 'parallel-test-orchestrator.md'),
    agentContent
  );
  ...
  const auditLogPath = path.join(__dirname, 'logs', 'audit.jsonl');
  try {
    const logContent = await fs.readFile(auditLogPath, 'utf-8');
    const logLines = logContent.trim().split('\n').map(line => JSON.parse(line));
    
    // Find parallel execution markers
    const parallelExecutions = logLines.filter(log => 
      log.content?.includes('[PARALLEL]') || 
      log.metadata?.groups?.some((g: any) => g.isConcurrent)
    );
    
    const sequentialExecutions = logLines.filter(log => 
      log.content?.includes('[SEQUENTIAL]')
    );
    
    const sidechainExecutions = logLines.filter(log => 
      log.content?.includes('[SIDECHAIN]') ||
      log.metadata?.isSidechain === true
    );
