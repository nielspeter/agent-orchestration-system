...
async function main() {
  ...
  const builder = AgentSystemBuilder.default()
    .withAgentsFrom('critical-illness-claim-structured/agents')
    .withToolsFrom('critical-illness-claim-structured/tools')
  ...
  const claimPath = path.join('critical-illness-claim-structured/claims/happy_path_claim.json');
