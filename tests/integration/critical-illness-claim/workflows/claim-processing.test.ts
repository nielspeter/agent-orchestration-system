...
/**
 * Critical Illness Claim Processing Integration Tests
 *
 * Tests the complete workflow for processing insurance claims through
 * multiple agents with real delegation (not simulation).
 *
...
describeWithFixtures(
  {
    name: 'Critical Illness Claim Processing',
    fixtureDir: 'tests/integration/critical-illness-claim/fixtures',
    ...
    generator: async (sessionId: string) => {
      ...
      const system = await AgentSystemBuilder.default()
        .withStorage(
          'filesystem',
          path.join(process.cwd(), 'tests/integration/critical-illness-claim/fixtures')
        )
      ...
      try {
        ...
        const claimPath = path.join(
          process.cwd(),
          'examples/critical-illness-claim/claims',
          scenario.claimFile
        );
