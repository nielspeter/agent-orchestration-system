...
export class AgentExecutor {
  ...
  async execute(
    ...
    this.logger.log({
      ...
      type: 'system',
      content: `Starting execution${execContext.parentAgent ? ` (delegated from ${execContext.parentAgent})` : ''}`,
      metadata: { 
    ...
    this.logger.log({
      ...
      metadata: { toolCount: Array.isArray(agent.tools) ? agent.tools.length : 'all' }
    });
    ...
    while (true) {
      ...
      this.logger.log({
        ...
        depth: execContext.depth,
        type: 'system',
        content: `Calling LLM (iteration ${iterationCount})`,
        metadata: { messageCount: messages.length, toolCount: tools.length }
      });
      
      const response = await this.llmProvider.complete(messages, tools);
