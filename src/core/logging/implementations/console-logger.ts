import { AgentLogger, ConsoleVerbosity } from '@/core/logging';
...
export class ConsoleLogger implements AgentLogger {
  ...
  private getAgentPrefix(agent: string): string {
    const depth = this.currentDepth.get(agent) || 0;

    // For nested agents, show the chain
    if (depth > 0) {
      return `  ${this.color(agent, 'dim')}: `;
    }
    return '';
  }
  ...
  logToolResult(agent: string, tool: string, _toolId: string, result: any): void {
  ...
  logDelegationComplete(parent: string, child: string, result: string): void {
    ...
    const childDepth = this.currentDepth.get(parent) || 0 + 1;
